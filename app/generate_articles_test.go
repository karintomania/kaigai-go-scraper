...
func TestGenerateArticle(t *testing.T) {
	...
	ag := NewArticleGenerator(pr, cr, WithGetImage(mockGetImage))
	...
	for i := 0; i < 28; i++ {
		require.Contains(t, article, fmt.Sprintf(`{{<matomeQuote body="コメント %d" userName="User-%d" createdAt="2025/02/02" color="">}}`, i, i))
	}
